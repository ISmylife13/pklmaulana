...
if(isset($_POST['save'])){
  ...
  $type2 = $type1[1];
  move_uploaded_file($tmp_name, '../foto-pakaian/'.$filename);
  if($NAMA && $DESKRIPSI && $ALAMAT && $CONTACT && $foto ){


    $sqli = "INSERT INTO tbl_pakaian (NAMA, DESKRIPSI, ALAMAT, CONTACT, FOTO) VALUES ('$NAMA', '$DESKRIPSI', '$ALAMAT', '$CONTACT', '$filename') ";
    $q1 = mysqli_query($koneksi, $sqli);
    if($q1){
      echo "<script> alert('Produk Berhasil Ditambahkan'); document.location.href = 'produk-pakaian.php'</script>";
    }else{
...
<h3 style="margin-top: 70px;">Tambah Produk Makanan</h3>
