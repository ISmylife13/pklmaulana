...
// edit 
$id = $_GET['ID'];
$ambil = mysqli_query($koneksi, "SELECT * FROM tbl_konter WHERE ID = $id");
...
if(isset($_POST['edit'])){
  ...
  if(!empty($lokasifoto)){
    move_uploaded_file($lokasifoto, "../foto-konter/$namafoto");

    mysqli_query($koneksi, "UPDATE tbl_konter SET NAMA ='$NAMA', DESKRIPSI='$DESKRIPSI', ALAMAT ='$ALAMAT', CONTACT ='$CONTACT', FOTO ='$namafoto' WHERE ID ='$id'");
  }else{

    mysqli_query($koneksi, "UPDATE tbl_konter SET NAMA ='$NAMA', DESKRIPSI='$DESKRIPSI', ALAMAT ='$ALAMAT', CONTACT ='$CONTACT'  WHERE ID ='$id'" );
  }
  echo "<script>alert ('Produk Berhasil Diedit');</script>";

  echo "<script>location='produk-konter.php';</script>";
