...
if(isset($_POST['save'])){
  ...
  $type2 = $type1[1];
  move_uploaded_file($tmp_name, '../foto-makanan/'.$filename);
  if($NAMA && $DESKRIPSI && $ALAMAT && $CONTACT && $foto ){


    $sqli = "INSERT INTO tbl_makanan (NAMA, DESKRIPSI, ALAMAT, CONTACT, FOTO) VALUES ('$NAMA', '$DESKRIPSI', '$ALAMAT', '$CONTACT', '$filename') ";
    $q1 = mysqli_query($koneksi, $sqli);
    if($q1){
      echo "<script> alert('Produk Berhasil Ditambahkan'); document.location.href = 'produk-makanan.php'</script>";
    }else{
